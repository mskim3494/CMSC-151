...
(define (conform-to-diet m)
  (cond
    [(empty? m) empty]
    [(cons? m)
     (meal-scale m (/ 25 (meal-fat m)))]));; the function should only scale if the fat is more than 25 gram otherwise it should be left unchaged
...
(define (within-circle? whatup notmuch)
  (cond 
    [(> (Circle-radius whatup) ;; should be >= instead of >
...
;; function above does not pass the following test
(: string-tree-h : (Rose-Tree String))
(define string-tree-h
  (make-Rose-Tree
   "A1"
   (list (make-Rose-Tree "A2" 
                         (list (make-Rose-Tree "A3" empty)
                               (make-Rose-Tree "A4" 
                                               (list (make-Rose-Tree "A5" empty)
                                                     (make-Rose-Tree "A6" empty)))))
         (make-Rose-Tree "A7"
                         (list (make-Rose-Tree "A8" empty)))
         (make-Rose-Tree "A9" empty))))
;;(check-expect (rose-height string-tree-h) 4)
...
;; problem 1 (concentric) (worth 5 if solved)    +5       

;; problem 4 (rose-map)   (worth 5 if solved)
(test)