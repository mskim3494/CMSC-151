...
;; your program did not pass the following tests


;(define graders-tests_new-board
;  (Board 
...
;         'none 'none 'none 'none 'none 'none 'none 'none)))
;
;(define graders-tests_new-game
;  (Game graders-tests_new-board 'black))
;
;;; a board with various outflanking moves available
;(define graders-tests_board0
;  (Board 
;   (list 'none 'none  'none  'none  'none  'none  'none  'none
;         'none 'white 'black 'black 'black 'black 'black 'none
;         'none 'black 'white 'white 'white 'white 'white 'none
;         'none 'black 'black 'black 'none  'black 'none  'none
;         'none 'white 'none  'none  'none  'none  'none  'none
;         'none 'black 'none  'none  'none  'none  'none  'none
;         'none 'white 'none  'none  'none  'none  'none  'none
;         'none 'none  'none  'none  'none  'none  'none  'none)))
;
;(define graders-tests_game0 (Game graders-tests_board0 'black))
;         
...
;(check-expect (outflanks? graders-tests_new-board 'black (Pos 7 7)) false)
;(check-expect (outflanks? graders-tests_new-board 'white (Pos 7 7)) false)

;; --- test black in every direction
;(check-expect (outflanks? graders-tests_board0 'black (Pos 1 0)) #t)
;(check-expect (outflanks? graders-tests_board0 'black (Pos 0 1)) #t)
;(check-expect (outflanks? graders-tests_board0 'black (Pos 7 1)) #t)
;(check-expect (outflanks? graders-tests_board0 'black (Pos 0 0)) #t)
;(check-expect (outflanks? graders-tests_board0 'black (Pos 3 7)) #t)


;; --- test white in every direction
;(check-expect (outflanks? graders-tests_board0 'white (Pos 4 5)) #t)
;(check-expect (outflanks? graders-tests_board0 'white (Pos 0 1)) #t)
;(check-expect (outflanks? graders-tests_board0 'white (Pos 0 7)) #t)

;; === test flips

(: graders-tests_sort-pos-list : (Listof Pos) -> (Listof Pos))
;; sort positions in lex order of "concatenated" row, col
;; n.b. wouldn't work if row and col were not constrained to [0,7]
(define (graders-tests_sort-pos-list ps) 
  (local
    {(: canon : Pos -> Integer)
     ;; "canonicalize" pos for use in sorting
     (define (canon p) (+ (* 10 (Pos-row p)) (Pos-col p)))
     (: pos< : Pos Pos -> Boolean)
     (define (pos< p q) (< (canon p) (canon q)))
     (: qs : (Listof Pos) -> (Listof Pos))
     (define (qs ps)
       (match ps
         ['() '()]
         [(cons piv t)
          (append 
           (qs (filter (λ ([p : Pos]) (pos< p piv)) t))
           (cons piv 
                 (qs (filter (λ ([p : Pos]) (not (pos< p piv))) t))))]))}
    (qs ps)))

;(check-expect (flips graders-tests_board0 'black (Pos 1 0))
;              (list (Pos 1 1)))

;(check-expect (flips graders-tests_board0 'black (Pos 0 1))
;              (list (Pos 1 1)))

;(check-expect (graders-tests_sort-pos-list
;               (flips graders-tests_board0 'black (Pos 0 0)))
;              (list (Pos 1 1) (Pos 2 2)))


;(check-expect (graders-tests_sort-pos-list
;               (flips graders-tests_board0 'white (Pos 0 2)))
;              (list (Pos 1 2) (Pos 1 3)))


;; === test game-over?

;(check-expect (game-over? graders-tests_new-game) #f)

;(check-expect 
; (game-over? 
;  (Game (Board (append (make-list 32 'black) (make-list 32 'none))) 'black)) 
; #t)

;(check-expect
; (game-over?
;  (Game (Board
;         (list 'white 'none  'none  'white  'none  'none  'white 'none
;               'none  'black 'none  'black  'none  'black 'none  'none
;               'none  'none  'black 'black  'black 'none  'none  'none
;               'white 'black 'black 'none   'black 'black 'black 'white
;               'none  'none  'black 'black  'black 'none  'none  'none
;               'none  'black 'none  'black  'none  'black 'none  'none
;               'white 'none  'none  'black  'none  'none  'black 'none
;               'none  'none  'none  'white  'none  'none  'none  'white))
;        'white))
;  #f)
...
;  (Game (Board
;         (list 'white 'none  'none  'white  'none  'none  'white 'none
;               'none  'white 'none  'white  'none  'white 'none  'none
;               'none  'none  'white 'white  'white 'none  'none  'none
;               'white 'white 'white 'white  'white 'white 'white 'white
;               'none  'none  'white 'white  'white 'none  'none  'none
;               'none  'white 'none  'white  'none  'white 'none  'none
;               'white 'none  'none  'white  'none  'none  'white 'none
;               'none  'none  'none  'white  'none  'none  'none  'white))
;        'black))
; #t)
...
;; outflanks?                        8/12 see comment at the end of the program
;; flips                             8/12
;; apply-move                        12/12
...
;; adequate tests                    4/ 8 could use more tests to catch mistakes

;; clarity (clear logic)             9/10

;; svn used correctly                2/ 2

;; _total-score_                    133/150
