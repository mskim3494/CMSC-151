...
(define (p2d-as-polar p)
  (cond
    ...
    ;;grader: if you're oing to divide by x or real-part p here, you need to account for
    ;;grader: when these are 0 to avoid divide by 0 errors
    [(polar-pair? p) p]))
(check-within 
...
;;grader: this issue can be solved by defining x instead as
;;grader: (define x (throw-dart (bounding-box a b)))
;;grader: which removes the problem of multiple throw-dart calls
(check-range
 (foo
  (make-Circle (make-Point 2 2) 3)
  (make-Circle (make-Point 5 3) 3)
...
(define origrt (polar-pair 0 0))
(define orign 0)
(define unit0xy (cartesian 1 0))
(define unit0rt (polar-pair 1 0))
(define unit0n 1)
(define unitp2xy (cartesian 0 1))
(define unitp2rt (polar-pair 1 (/ pi 2)))
(define unitp2n 0+i)
(define unitpxy (cartesian -1 0))
(define unitprt (polar-pair 1 pi))
(define unitpn -1)
(define unit3p2xy (cartesian 0 -1))
(define unit3p2rt (polar-pair 1 (/ (* 3 pi) 2)))
(define unit3p2n 0-i)
...
;; problem 3 (p2d-as-polar)      1.75/ 2 (see comment under function)
;; problem 3 (p2d-x)                2/ 2
;; problem 3 (p2d-y)                1/ 1
;; problem 3 (p2d-n)                2/ 2
;; problem 3 (p2d-r)                2/ 2
;; problem 3 (p2d-theta)         1.75/ 2 (same issue as p2d-as-polar)
;; problem 3 (p2d-dist)             2/ 2

;; problem 4 (throw-dart)           2/ 2
;; problem 4 (within-circle?)       2/ 2
;; problem 4 (bounding-box)         2/ 2
;; problem 4 (area-of-intersection) 3/ 4 (see comment under foo)
;; problem 4 ({left,right}-circle)  1/ 1
;; problem 4 (venn-monte-carlo)     1/ 1
