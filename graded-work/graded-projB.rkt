...
(define (parse-pos s)
  (define p
    (map
     ...
     (string->list s)))
  (cond
    [(eqv? #f (string->number s)) "unparseable"]  ;; GRADER: should return s here
  ...
  (local
     ...
     (define (loop g b w x)
        (if
          (game-over? g) '()
          (if
          (eqv? (can-move? g) #f) ;; GRADER: testing (not (can-move? g)) is more direct!
           (loop (Game (Game-board g) (op (Game-next g))) b w x)
           (local 
             {(define strategy (if (eqv? (Game-next g) 'black) (b g) (w g)))}
             (cond
               [(eqv? x 3) '()]  ;; GRADER: supposed to signal an error in this case
...
;; GRADER: note that you could combine the map and argmax to get a more efficient function
...
;; GRADER: function name is not very meaningful
;; GRADER: more efficient edge test is to compare row and col against 0 and 7
        ...
        (local
          {(define next (next-pos g))
           (define arg (if (eqv? 'black (Game-next g)) argmax argmin))}
              (Pair-x2
;; GRADER: more efficient to combine map lambda with argmin/argmax lambda
...
(: graders_take : (All (α) Integer (Listof α) -> (Listof α)))
;; take up to n items; no error if list is too short
(define (graders_take n xs)
  (if (or (zero? n) (empty? xs)) 
      '() 
      (cons (first xs) (graders_take (sub1 n) (rest xs)))))

(define-type Stringish (U Symbol String))

(: graders_quick_board : Stringish * -> Board)
;; can quickly throw together a board this way
;; - builds a list of length 64 in all cases
;; - w W mean white, b B mean black, . means none, e means empty row
;; - input is zero or more strings and/or symbols; they are concatenated together
;; ex: (grader_quick_board '.wwb) => (Board (list 'none 'white 'white 'black --58 'nones--))
;; ex: (grader_quick_board '. 'w 'w 'b), (grader_quick_board ".wwb"), (grader_quick_board ".wwb....") all equiv.
(define (graders_quick_board . ss)
  (define 8-nones (make-list 8 'none))
  (: str : Stringish -> String)
  (define (str x) 
    (if (symbol? x) (symbol->string x) x))    
  (: f : Stringish String -> String)
  (define (f x t) (string-append (str x) t))
  (: lp : (Listof Char) -> (Listof (U Player 'none)))
  (define (lp cs)
    (match cs
      ['() '()]
      [(cons #\. ct) (cons 'none (lp ct))]
      [(cons (or #\e #\E) ct) (append 8-nones (lp ct))]
      [(cons (or #\b #\B) ct) (cons 'black (lp ct))]
      [(cons (or #\w #\W) ct) (cons 'white (lp ct))]))
  (define r (lp (string->list (foldr f "" ss))))
  (Board (append (graders_take 64 r)
                 (make-list (max 0 (- 64 (length r))) 'none))))

(: graders_quick_game : (U 'b 'w) Stringish * -> Game)
;; assemble a game from next player and quick_board style arguments
(define (graders_quick_game n . ss)
  (Game (apply graders_quick_board ss)
        (match n ['b 'black] ['w 'white])))

(: graders_p1 : (All (a b) (Pair a b) -> a))
;; because we don't know what the student's pair fields are named
(define (graders_p1 p)
  (match p [(Pair a b) a]))

(: graders_p2 : (All (a b) (Pair a b) -> b))
;; because we don't know what the student's pair fields are named
(define (graders_p2 p)
  (match p [(Pair a b) b]))
...
(define graders_game_1
  (Game (Board (list 'none 'none 'none 'none 'none 'none 'none 'none
...
(define graders_game_2
  (Game (Board (list 'none 'none 'none 'none 'none 'none 'none 'none
                     'white 'none 'none 'none 'none 'none 'none 'none
                     'white 'none 'none 'none 'none 'none 'none 'none
                     'black 'none 'none 'none 'none 'none 'none 'black
                     'none 'none 'none 'none 'none 'none 'none 'white
                     'none 'none 'none 'none 'none 'none 'none 'white
                     'none 'none 'none 'none 'none 'none 'none 'white
                     'none 'none 'none 'none 'none 'none 'none 'none))
...
(define graders_game_3
  (Game (Board (list 'white 'none 'none 'none 'none 'none 'none 'none
                     'white 'white 'none 'none 'none 'none 'none 'none
                     'white 'none 'none 'none 'none 'none 'none 'none
                     'black 'none 'none 'none 'none 'none 'none 'black
                     'none 'none 'none 'none 'none 'none 'none 'white
                     'none 'none 'none 'none 'none 'none 'none 'white
...
(define graders_game_4
  (Game (Board (list 'none 'none 'none 'black 'none 'none 'none 'none
...
(define graders_game_5
  (graders_quick_game 'w 
                      '.....wb.
                      '.bw.....
                      '....wb..))
...
;; test first-move
(check-expect (first-move new-game) (Pos 2 3))
(check-expect (first-move graders_game_1) (Pos 0 0))
(check-expect (first-move graders_game_2) (Pos 0 0))
(check-expect (first-move graders_game_3) (Pos 7 7))
(check-expect (first-move graders_game_4) (Pos 7 3))

;; test play-loop

(define graders_first-vs-first (play-loop new-game first-move first-move))
(check-expect 
 (Game-board (graders_p2 graders_first-vs-first))
 (graders_quick_board 'wwwwwwwb
                      'wwwwwwbb
                      'wwwwwbwb
                      'wwwwbwwb
                      'wwwwwwwb
...
(check-expect (immediate-tactics graders_game_1) (Pos 0 0))
(check-expect (immediate-tactics graders_game_2) (Pos 7 7))
(check-expect (immediate-tactics graders_game_3) (Pos 7 7))
(check-expect (immediate-tactics graders_game_4) (Pos 7 3))
(check-expect (immediate-tactics graders_game_5) (Pos 0 7))

;; piece-counting
(check-expect (piece-counting new-game) 0)
(check-expect (piece-counting graders_game_1) 0)
(check-expect (piece-counting graders_game_2) -3)
(check-expect (piece-counting graders_game_3) -5)

;; prefer-edges
(check-expect ((prefer-edges 2) new-game) 0)
(check-expect ((prefer-edges 2) graders_game_1) 0)
(check-expect ((prefer-edges 2) graders_game_2) -6)
(check-expect ((prefer-edges 2) graders_game_3) -9)

;; prefer-edges-and-corners
(check-expect ((prefer-edges-and-corners 2 3) new-game) 0)
(check-expect ((prefer-edges-and-corners 2 3) graders_game_1) 0)
(check-expect ((prefer-edges-and-corners 2 3) graders_game_2) -6)
(check-expect ((prefer-edges-and-corners 2 3) graders_game_3) -10)

;; minimax-eval
(check-expect (minimax-eval piece-counting 0 new-game) 0)
(check-expect (minimax-eval piece-counting 0 graders_game_4) -5)
(check-expect (minimax-eval piece-counting 1 graders_game_4) 8)

;; test human
(display "\ntesting \"human\": type 23 at the prompt\n")
(check-expect (human new-game) (Pos 2 3))

;; minimax
;; -- test by playing --
; (play-loop new-game human (minimax piece-counting 1))
; (play-loop new-game human (minimax piece-counting 5))
; (play-loop new-game human (minimax (prefer-edges-and-corners 3 9) 5))

;; montymax
;; -- test by playing --
; (play-loop new-game human (montymax piece-counting 1 1))
; (play-loop new-game human (montymax piece-counting 3 1))
; (play-loop new-game human (montymax piece-counting 3 3))
; (play-loop new-game human (montymax (prefer-edges-and-corners 3 9) 3 5))
 
;; evaluation

;; Very good!!
;; all tests passed
;; see comments labeled "GRADER:" in code for more details

;; === correctness ===

;; first-move                6/ 6
;; immediate-tactics         9/10  ;; efficiency

;; human (parse-pos)         9/10  ;; parse-pos does not meet spec
;; play-loop                 9/10  ;; parse-pos does not meet spec
...
;; code layout                       6/ 8  ;; some lines too long
;; identifiers are well named        6/ 8
